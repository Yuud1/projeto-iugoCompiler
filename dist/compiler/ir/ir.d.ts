/**
 * ir.ts — Representação Intermediária (IR) do compilador iuGo
 *
 * Implementa Three-Address Code (TAC / 3AC) — representação clássica
 * usada em compiladores reais (GCC GIMPLE, LLVM IR, JVM bytecode).
 *
 * Cada instrução tem no máximo três "endereços" (operandos):
 *
 *   dest = src1  operator  src2
 *
 * Um "endereço" (place) pode ser:
 *   - Nome de variável do programa: x, idade
 *   - Temporário gerado pelo compilador: t0, t1, t2, …
 *   - Constante literal: 20, "Maior", true
 *
 * ── Formato textual de cada instrução ──────────────────────────────────────
 *
 *   assign     t0 = 20               (cópia / literal)
 *   binop      t1 = idade + 1        (operação binária)
 *   unop       t2 = -x               (operação unária)
 *   label      L0:                   (rótulo de destino para saltos)
 *   jump       goto L0               (salto incondicional)
 *   iffalse    iffalse t1 goto L2    (salto condicional — se falso, salta)
 *   print      print t0              (instrução de saída)
 *
 * ── Exemplo completo ────────────────────────────────────────────────────────
 *
 * Fonte iuGo:
 *   let idade = 20;
 *   if (idade >= 18) { print("Maior"); }
 *
 * TAC gerado:
 *   idade = 20
 *   t0 = idade >= 18
 *   iffalse t0 goto L0
 *   print "Maior"
 *   L0:
 */
/** x = src  (atribuição / cópia de variável ou literal) */
export interface AssignInstr {
    kind: 'assign';
    dest: string;
    src: string;
}
/** dest = left op right  (operação binária: aritmética, comparação, lógica) */
export interface BinOpInstr {
    kind: 'binop';
    dest: string;
    op: string;
    left: string;
    right: string;
}
/** dest = op operand  (operação unária: negação numérica ou lógica) */
export interface UnOpInstr {
    kind: 'unop';
    dest: string;
    op: string;
    operand: string;
}
/** Lx:  (rótulo — destino de saltos) */
export interface LabelInstr {
    kind: 'label';
    name: string;
}
/** goto Lx  (salto incondicional) */
export interface JumpInstr {
    kind: 'jump';
    target: string;
}
/** iffalse cond goto Lx  (salto se condição for falsa) */
export interface IfFalseInstr {
    kind: 'iffalse';
    cond: string;
    target: string;
}
/** print value  (saída de dados — built-in da linguagem iuGo) */
export interface PrintInstr {
    kind: 'print';
    value: string;
}
export type IRInstr = AssignInstr | BinOpInstr | UnOpInstr | LabelInstr | JumpInstr | IfFalseInstr | PrintInstr;
/** Programa completo em Three-Address Code. */
export interface IRProgram {
    instructions: IRInstr[];
}
/**
 * Converte o IRProgram em texto legível para exibição na IDE.
 * Labels ficam alinhados à esquerda; demais instruções são indentadas.
 */
export declare function formatIR(program: IRProgram): string;
//# sourceMappingURL=ir.d.ts.map