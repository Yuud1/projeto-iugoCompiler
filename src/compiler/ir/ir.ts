...
/** x = src  (atribuição / cópia de variável ou literal) */
export interface AssignInstr  { kind: 'assign';  dest: string; src: string }

/** dest = left op right  (operação binária: aritmética, comparação, lógica) */
export interface BinOpInstr   { kind: 'binop';   dest: string; op: string; left: string; right: string }

/** dest = op operand  (operação unária: negação numérica ou lógica) */
export interface UnOpInstr    { kind: 'unop';    dest: string; op: string; operand: string }

/** Lx:  (rótulo — destino de saltos) */
export interface LabelInstr   { kind: 'label';   name: string }

/** goto Lx  (salto incondicional) */
export interface JumpInstr    { kind: 'jump';    target: string }

/** iffalse cond goto Lx  (salto se condição for falsa) */
export interface IfFalseInstr { kind: 'iffalse'; cond: string; target: string }

/** print value  (saída de dados — built-in da linguagem iuGo) */
export interface PrintInstr   { kind: 'print';   value: string }
...
/** Programa completo em Three-Address Code. */
